...
Client.prototype.vote = function(callback) {
    if (typeof callback !== 'undefined') {this.vote.voteCallback = callback;}
}
Client.prototype.vote.addoption = function(title) {
    if (typeof this.votes === 'undefined') {this.votes = []; this.voteCount = [];}
    this.votes.push(title);
    this.voteCount.push(0);
}
Client.prototype.vote.options = function(arr) {
    this.votes = arr;
    this.voteCount = [];
...
Client.prototype.vote.add = function(index,user) {
    if (typeof this.voteStarted == 'undefined' || !this.voteStarted) {return;}
    if (typeof this.voted === 'undefined') {this.voted = [];}
    if (this.voted.indexOf(user) == -1 && index <= this.votes.length) {
        this.voteCount[parseInt(index)-1] += 1;
        this.voted.push(user);
    }
}
Client.prototype.vote.message = function(msg) {
    this.voteMessage = msg;
}
Client.prototype.vote.completeMessage = function(msg) {
    this.voteCompleteMessage = msg;
}
Client.prototype.vote.channel = function(msg) {
    this.voteChannel = msg;
}
Client.prototype.vote.optionMessage = function(msg) {
    this.voteOptionMessage = msg;
}
Client.prototype.vote.inProgressMessage = function(msg) {
    this.voteInProgressMessage = msg;
}
Client.prototype.vote.timeout = function(time) {
    this.voteTimeout = time;
}
Client.prototype.vote.messageCount = function(time) {
    this.voteMessageCount = time;
}
Client.prototype.vote.command = function(c) {
    this.voteCommand = c;
}
Client.prototype.vote.start = function(title) {
    if (typeof this.votes === 'undefined') {return;}
    if (this.votes.length <= 1) {return;}
    if (typeof this.voteTimeout === 'undefined') {this.voteTimeout = 120;}
    if (typeof this.voteMessage === 'undefined') {this.voteMessage = 'Voting for the next pro will now begin:';}
    if (typeof this.voteInProgressMessage === 'undefined') {this.voteInProgressMessage = 'Voting is in progress for the next pro:';}
    if (typeof this.voteCompleteMessage === 'undefined') {this.voteCompleteMessage = 'Voting has ended. Stay tuned to see who won.';}
    if (typeof this.voteOptionMessage === 'undefined') {this.voteOptionMessage = '%index%) %option%';}
    if (typeof this.voteMessageCount === 'undefined') {this.voteMessageCount = 4;}
    if (typeof this.voteChannel === 'undefined') {this.voteChannel = '#leagueproslive';}

    this.voteStarted = true;
    if (typeof this.voteCommand == 'undefined') {this.voteCommand = '#';}

    this.parent.say(this.voteChannel, this.voteMessage);
    printVoteOptions(this);

    var _this = this;
    var displayCount = 1;
    var interval = setInterval(function(){
        if (displayCount < _this.voteMessageCount-1) {
            _this.parent.say(_this.voteChannel, _this.voteInProgressMessage);
            printVoteOptions(_this);
        } else {
            var winner = 0;
            var max = 0;
            for (var i = 0; i < _this.votes.length; i++) {
                if (_this.voteCount[i] > max) {winner = i; max = _this.voteCount[i];}
            }
            
            _this.parent.say(_this.voteChannel, _this.voteCompleteMessage.replace('%winner%', _this.votes[winner]));
            _this.winner = _this.votes[winner];
            clearInterval(interval);
            _this.reset();
            _this.voteCallback.call();
        }
        displayCount++;
    },(_this.voteTimeout*1000)/_this.voteMessageCount);

    function printVoteOptions(_this) {
        var type = '';
        for (var item in _this.votes) {
            var msg = _this.voteOptionMessage.replace('%index%', parseInt(item)+1);
            msg = msg.replace('%option%', _this.votes[item]);
            _this.parent.say(_this.voteChannel, msg);
            if (item < _this.votes.length-2) {
                type += ' '+(parseInt(item)+1)+ ',';
            } else if (item == _this.votes.length - 2) {
                type += ' '+(parseInt(item)+1);
            } else {
                type += ' or '+(parseInt(item)+1);
            }
        }

        _this.parent.say(_this.voteChannel, 'Type'+type+' to vote.');
    }
}
Client.prototype.vote.reset = function() {
    this.votes = [];
    this.voteCount = [];
    this.voted = [];
    this.voteStarted = false;
}
Client.prototype.winner = function() {
    var temp = this.vote.winner;
    this.vote.winner = undefined;
    return temp !== 'undefined' ? temp : '';
}
Client.prototype.setRunes = function(msg) {
    this.runes = msg;
}
Client.prototype.setMasteries = function(msg) {
    this.masteries = msg;
}
Client.prototype.setChampion = function(msg) {
    this.champion = msg;
}
Client.prototype.setSummoner = function(msg) {
    this.summoner = msg;
}
/*
